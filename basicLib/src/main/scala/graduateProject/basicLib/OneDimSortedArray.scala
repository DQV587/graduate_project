...
class OneDimSortedArray[C1, C2](input: Array[Array[Any]],
                                ...
                                compareFun1: (C1, C1) => Boolean,
                                compareFun2: (C2, C2) => Boolean) extends java.io.Serializable {
    ...
    val content = input.sortWith((x, y) => compareFun1(getValueFun1(x),getValueFun1(y)))
        ...
        override def hasNext: Boolean = {
            if (nextElement != null)
                ...
                while (currentRowIndex < content.length && nextElement == null) {
                    ...
                    if (compareFun1(getValueFun1(row), keyValue1)) {
                        if (compareFun2(getValueFun2(row), keyValue2)) {
                            nextElement = row
                        }
                    } else {
    ...
    def getMin: Array[(C1, C2)] = {
        ...
        for (i <- 1 until size) {
          val curEle=content(i)
            if (compareFun2(result(i - 1)._2, getValueFun2(curEle))) {
